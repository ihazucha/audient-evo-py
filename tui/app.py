...
class EvoTUI:
    ...
    def _refresh(self):
        try:
            self.state = {
                ...
                "gain": [self.evo.get_gain_debug("input1"), self.evo.get_gain_debug("input2")],
                "mix": self.evo.get_mix(),
                "mute": {t: self.evo.get_mute(t) for t in ("input1", "input2", "output")},
                "phantom": {t: self.evo.get_phantom(t) for t in ("input1", "input2")},
            }
        except OSError as e:
    ...
    def _adjust(self, delta):
        ...
        try:
            if etype == "volume":
                ...
                self.evo.set_gain(f"input{target}", new)
            else:
    ...
    def _set_value(self, val):
        ...
        try:
            if etype == "volume":
                ...
                self.evo.set_gain(f"input{target}", val)
            else:
    ...
    def _draw_monitor_section(self, scr, row, cx):
        ...
        self._box_top(scr, row, cx, "MONITOR", active)
        row += 1
        pct, _ = self._val(0)
        ...
        row += 1
        self._box_side(scr, row, cx)
        self._safe(scr, row, cx + SLIDER_OFF, f"Mix:  {pct:3d}%", curses.A_BOLD if active else 0)
        row += 1
    ...
    def _draw_output_section(self, scr, row, cx):
        ...
        self._box_top(scr, row, cx, "OUTPUT", active)
        row += 1
        pct, db = self._val(1)
    ...
    def _draw_input_section(self, scr, row, cx, ch):
        ...
        self._box_top(scr, row, cx, f"INPUT {ch + 1}", active)
        row += 1
        pct, db = self._val(ei)
    ...
    def _draw(self, scr):
        ...
        row = self._draw_monitor_section(scr, row, cx)
        row += 1
        row = self._draw_output_section(scr, row, cx)
        row += 1
        row = self._draw_input_section(scr, row, cx, 0)
        row += 1
        ...
        if self.status or self.num_buf:
            self._box_attr = curses.color_pair(C_WHITE) | curses.A_DIM
            self._box_top_plain(scr, row, cx)
            row += 1
            self._box_side(scr, row, cx)
            if self.num_buf:
                self._safe(
